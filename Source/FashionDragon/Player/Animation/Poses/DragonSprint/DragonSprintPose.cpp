...
void FDragonSprintLegDriver::Tick(const float DeltaTime)
{
	...
	const auto ShouldDisconnect = FMath::Abs(LegReference.Y) > 100.f || LegReference.Z < -150.0f || LegReference.X > 600.0f || LegReference.X < -300.0f
...
FDragonSprintPose::FDragonSprintPose(UDragonAnimInstance* Anim): FProceduralPose(Anim)
{
	...
	NeckTurnDriver->SetAxisMask(FVector(0, 1.0, -0.3));
