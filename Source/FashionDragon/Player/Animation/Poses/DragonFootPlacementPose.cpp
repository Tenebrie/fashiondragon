...
FPoseEffector FDragonFootPlacementLegDriver::ToEffector(const FPoseEffector& BaseEffector, const FPoseEffectorContext& Context)
{
	...
	const auto PredictedWorldPosition = Leg->GetWorldPosition(BaseEffector) - FootForward.RotateVector(FVector(0, 50, 0));
	
	FVector Start = PredictedWorldPosition + FVector(0, 0, 100.0f);
	...
	auto FootTip = PredictedWorldPosition + FootForward.RotateVector(FVector(0, 100, 0));
	...
	if (bHitOnFootBase && bHitOnFootTip)
	{
		...
		const FVector WorldPos = HitResultA.ImpactPoint.Z > HitResultB.ImpactPoint.Z
			...
			: FootTip;
		return BaseEffector
			.AddPosition(GroundPosition - WorldPos);
	}
	if (bHitOnFootBase)
	{
		const FVector GroundPosition = HitResultA.ImpactPoint;
		return BaseEffector
			.AddPosition(GroundPosition - PredictedWorldPosition);
	}
	if (bHitOnFootTip)
	{
		const FVector GroundPosition = HitResultB.ImpactPoint;
		return BaseEffector
			.AddPosition(GroundPosition - FootTip);
	}
